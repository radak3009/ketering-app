                  ...
                  {filteredMeals
                    ...
                    >
                      <div className="flex items-start gap-3">
                        {meal.image_url ? (
                          <img src={meal.image_url} alt={meal.name} className="w-12 h-12 rounded object-cover shrink-0" />
                        ) : (
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <Badge variant={meal.status === 'aktivan' ? 'default' : 'secondary'} className="text-xs shrink-0">
                              {meal.status}
                            </Badge>
                          </div>
                          <div className="flex flex-wrap items-center gap-1.5 mt-1">
                            {meal.code && <span className="text-xs font-mono text-muted-foreground">{meal.code}</span>}
                            {meal.meal_group && <Badge variant="outline" className="text-xs h-5">{meal.meal_group}</Badge>}
                            {meal.shifts && meal.shifts.length > 0 && meal.shifts.map((shift, i) => (
                              <Badge key={i} variant="secondary" className="text-xs h-5">{getShiftRoman(shift)}</Badge>
                            ))}
                          </div>
                          {meal.price && (
                            <p className="text-xs text-muted-foreground mt-1">{meal.price} RSD</p>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          {selectedMeal && (
            <div className="space-y-4 mt-6">
              ...
              <div>
                <Label>Grupa</Label>
                <GroupCombobox
                  value={selectedMeal.meal_group || ''}
                  onChange={(g) => setSelectedMeal({ ...selectedMeal, meal_group: g })}
                  options={availableGroups}
                  onCreate={async (name) => {
                    const g = normalizeGroupName(name);
                    if (!g) return null;
